...
/*
How it works:
  WMO weather codes are numbers the Open-Meteo API returns to describe conditions.
  We map each code to a Material Icon name and a plain-English description.
  The icon name goes into the <span class="material-icons"> element in the HTML.
*/
const weatherCodeMap = {
  ...
  55: { icon: 'water_drop',        desc: 'Heavy drizzle' },
  61: { icon: 'water_drop',        desc: 'Light rain' },
  63: { icon: 'rainy',             desc: 'Moderate rain' },
  65: { icon: 'rainy',             desc: 'Heavy rain' },
  71: { icon: 'ac_unit',           desc: 'Light snow' },
  73: { icon: 'ac_unit',           desc: 'Moderate snow' },
  75: { icon: 'severe_cold',       desc: 'Heavy snow' },
  80: { icon: 'rainy',             desc: 'Rain showers' },
  81: { icon: 'thunderstorm',      desc: 'Heavy showers' },
...
function getWeather() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    function(position) {
      ...
      //Docs used: https://open-meteo.com/en/docs
      const url =
      ...
      fetch(url)
        ...
        .then(function(data) {
          ...
          //If the code isn't in our map, fall back to a generic thermometer icon
          const weather  = weatherCodeMap[code] || { icon: 'device_thermostat', desc: 'Unknown' };
    ...
    function(error) {
      ...
      document.getElementById('weather-icon').textContent = 'location_off'; // Material Icon name
    }
  );
}
...
/*
   3. RANDOM QUOTE
   First we had used Quotable but we switched from Quotable (api.quotable.io) because it shut down.
   We now use ZenQuotes (zenquotes.io/api/random) which is free, no key needed.
*/

//A few built-in quotes used as a backup if the API or proxy is unreachable
...
function fetchQuote() {
  ...
  //Wrap the ZenQuotes URL inside the allorigins proxy URL
  //encodeURIComponent makes the URL safe to pass as a query parameter
  const zenUrl   = 'https://zenquotes.io/api/random';
  const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(zenUrl)}`;

  fetch(proxyUrl)
    .then(function(response) {
      return response.json(); //parse the outer proxy response first
    })
    .then(function(data) {
      //data.contents is a string — we parse it again to get the actual quote array
      const quoteArray = JSON.parse(data.contents);
      const quote      = quoteArray[0]; // ZenQuotes always returns an array; we take item [0]

      document.getElementById('quote-text').textContent   = `"${quote.q}"`;
      document.getElementById('quote-author').textContent = `— ${quote.a}`;
    })
    .catch(function(error) {
      //If the API or proxy is down, show a random fallback quote instead
...
/* 
   ...
   We convert our array to and from JSON string when saving/loading.
...
//Display/render todos on page load (loads saved data from localStorage)
renderTodos();
